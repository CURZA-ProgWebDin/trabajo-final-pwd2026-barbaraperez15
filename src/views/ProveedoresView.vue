  ...
  <div class="view-container">
    ...
    <form v-if="showForm" @submit.prevent="handleSave" class="form">
      ...
      <div class="form-group">
        ...
        <input v-model="formData.contacto" id="contacto" />
      </div>
      <div class="form-group">
        <label for="telefono">Teléfono:</label>
        <input v-model="formData.telefono" id="telefono" />
      </div>
      <div class="form-group">
    ...
    <table class="table" v-if="!loading">
      ...
      <tbody>
        <tr v-for="prov in proveedoresStore.proveedores" :key="prov.id">
          <td>{{ prov.nombre }}</td>
          <td>{{ prov.contacto || '-' }}</td>
          <td>{{ prov.telefono || '-' }}</td>
          <td>{{ prov.email || '-' }}</td>
          <td>
            ...
            <button @click="deleteProveedor(prov.id)" class="btn-delete">Eliminar</button>
          </td>
        </tr>
      </tbody>
    </table>
...
const formData = ref({ nombre: '', contacto: '', telefono: '', email: '', id: null })
...
const handleSave = async () => {
  ...
  try {
    if (formData.value.id) {
      ...
      await proveedoresStore.updateProveedor(id, data)
      success.value = 'Proveedor actualizado'
    } else {
      const { id, ...data } = formData.value
      await proveedoresStore.createProveedor(data)
      success.value = 'Proveedor creado'
    }
    formData.value = { nombre: '', contacto: '', telefono: '', email: '', id: null }
